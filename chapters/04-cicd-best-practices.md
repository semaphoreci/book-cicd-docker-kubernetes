...
- Container registry
- Connections to multiple environments (staging, production...)
- Audit log
...
In the [seminal 2006 interview to ACM](https://queue.acm.org/detail.cfm?id=1142065), Werner Vogels, Amazon CTO, pioneered the mindset of *you build it, you run it*. The idea is that developers should be in direct contact with the operation of their software. That way they're in close contact with customers. 

The key insight is that involving developers in the customer feedback loop is essential for improving the quality of the service. Which ultimately leads to better business results.
...
Besides speed, there is the aspect of reliability. The goal is to be sure that every automated test ran against the artifact which will go to production.
...
This ensures that the build doesn't change as it progresses through the system.
...
No one may touch the service's repository for months. And then, one day, there's an urgent need to deploy a change. The CI build fails with unexpected errors. There are security vulnerabilities in several dependencies. Some dependencies have introduced breaking changes. What seemed like a trivial update becomes a high-risk operation that may drag into days of work.

To prevent this from happening, you can schedule a daily CI build. A scheduled build is a great way of detecting any issues with dependencies early, regardless of how often your code changes (or doesn't).
